...
def is_valid_template(template_dir: Path) -> bool:
    ...
    if (
        not (prompts_dir / "selection.md").exists()
        or not (prompts_dir / "usage.md").exists()
    ):
        return False
...
def process_template(template_dir: Path) -> Dict[str, Any]:
    ...
    return {
        ...
        "description": {"selection": selection_content, "usage": usage_content},
    }
